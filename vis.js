...
var drawBasemap = function(json) {
    ...
    basemap.on("mouseover.tooltip", function(d) {
        ...
        tooltip.text(
            nhood + ": " + calculateRatio(area.values[nhood], area.total)
        );
...
var drawRecords = function(data) {
    ...
    symbols.on("mouseout.details", function(d) {
        ...
        g.piechar.select(".pieCount").text("Count: " + recordCount.total);

        g.piechar.select(".pieRatio").text("Ratio: 100%");
    });
...
svg.append("text") // Pie chart title
    ...
    .attr(
        "transform",
        translate(attr.plotWidth - attr.margin.right, attr.margin.top + 145)
    )
    .text("Ratio");
...
var drawPie = function(recordCount) {
    ...
    g.piechar.attr(
        "transform",
        translate(attr.plotWidth - 90, attr.margin.top + 215)
    ); // Moving chart to upper-right corner

    g.piechar
        .selectAll(".arc") // Drawing Pie
        .data(pie(map))
        .enter()
        .append("path")
        .attr("class", "arc")
    ...
    arcs.on("mouseover.arc", function(d) {
        ...
        symbols
            .filter(e => e.Source != d.data.source)
            .lower()
            .transition()
            .duration(200)
            .style("fill", "#555555")
        ...
        g.piechar.select(".pieCount").text("Count: " + d.data.value);

        g.piechar
            .select(".pieRatio")
            .text("Ratio: " + calculateRatio(d.data.value, recordCount.total));
    });
...
var drawLegends = function(data) {
    g.legends
        ...
        .attr(
            "transform",
            translate(attr.plotWidth - attr.margin.right, attr.margin.top - 5)
        )
        .text("Source");

    let legend = g.legends
        .selectAll("g") // Legend rows
        ...
        .attr("transform", function(d, i) {
            return translate(
                attr.plotWidth - attr.margin.right,
                attr.margin.top + i * 20
            );
        });

    legend
        .append("rect") // Rect for interactivity
    ...
    legends.on("mouseover.brushing", function(d) {
        ...
        g.piechar.select(".pieCount").text("Count: " + value);
    ...
    legends.on("mouseout.brushing", function(d) {
        ...
        g.piechar.select(".pieCount").text("Count: " + total);
...
g.dslider
    .append("text") // Slider title
    .attr("class", "bold")
    .attr("text-anchor", "start")
    .attr(
        "transform",
        translate(attr.plotWidth - attr.margin.right, attr.coordinate.y1 + 4)
    )
    .text("Opened");
